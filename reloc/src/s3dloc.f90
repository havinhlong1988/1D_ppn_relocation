...
!   History record file
    ...
    !-residual check file annotations
        write(1,*)"/iteration,rms, adjustment, tshift, x0, y0, z0/"
...
!       write(1,"(a)")"==================*========================"
       ...
       open(8,file='dataout.chk',status='unknown') ! report good data output
       open(9,file='location.chk',status='unknown') ! Compare input and output location
!       open(10,file="stacor.log",status="unknown")
       open(22,file='runtime.log',status='unknown') ! runtime report
...
!       print*,sta_nm(sta_idx(1)),abs_tt(1) 
       ...
       do ievt=1,nevt
          ...
          write(1,*)"#","eventid:",evt_id(evtnow),", number of input ray: ",nrds
!         print*,nrds,evt_pack(ievt)+1,evt_pack(ievt+1)
          ...
          if(z0.gt.dm) cycle ! skip eq forcal depper than moho
...
!                write(*,"(a24,3g15.5)")"* researching minimum:",x0,y0,z0
...
!                itr=0  ! reset iteration count
          ...
          if (iflag.ne.1) then
            ...
            write(8,50)evt_date(evtnow),evt_time(evtnow),evt_loc(evtnow,1),evt_loc(evtnow,2),evt_loc(evtnow,3),evt_mag(evtnow),&
            0.0,0.0,0.0,evt_id(evtnow),y0,x0,z0,iflag
...
!        write(*,"(a4,1x,f8.4,i3)")sta_nm(i),sta_cor(i,j),j ! station write out
...
!          if(itr==maxitr) then
          ...
          enddo
          do ii=ie,ie+is-1
               write(idp,'(3f10.4,3x,i1)')(w(j,(2*ie+is-ii)),j=2,3),(w(1,ie+(ie+is-ii))),ray_pha(i)
          enddo
!         endif
       else
...
!    print*,"rayused:",nrow,
    ...
    !-output check file
    write(1,"(i4,i6,3f10.5,3f15.3)")itr,nrow,rms,adj,dt,ex,geoc_to_geog(ey),ez
...
!   Date modified: 2019/11/25 19:50 - 
       if((ez.gt.dm).or.(ez.lt.0))  iflag=4
       return
    endif
!   do i=1,nrow
!      Wd(i,i)=1.0
